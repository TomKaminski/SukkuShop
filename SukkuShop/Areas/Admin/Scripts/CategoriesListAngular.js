...
adminApp.controller("AdminCategoriesCtrl", ["$scope", "$http", '$filter', function ($scope, $http, $filter) {
    ...
    $scope.newSubCategoryDiscount = "";
    $scope.newEditorName = "";
    $scope.newEditorDiscount = "0";
    ...
    $scope.discountEditChange = function (discount) {
        if (discount > 100)
            $scope.newSubCategoryDiscount = "100";
    }
    ...
    $scope.validateCategoryDiscountForm = function (name, discount) {
